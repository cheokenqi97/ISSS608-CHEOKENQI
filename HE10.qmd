...
# Lesson 10: Information Dashboard Design: R methods
...
```{r eval=FALSE}
library(RODBC)
con <- odbcConnectAccess2007('chap10/data/Coffee Chain.mdb')
coffeechain <- sqlFetch(con, 'CoffeeChain Query')
write_rds(coffeechain, "chap10/data/CoffeeChain.rds")
odbcClose(con)
...
```{r}
coffeechain <- read_rds("chap10/data/CoffeeChain.rds")
```
## 31.3.3 Aggregating Sales and Budgeted Sales at Product level
```{r}
product <- coffeechain %>%
  group_by(`Product`) %>%
...
we need to install dataui R package by using the code chunk below.
